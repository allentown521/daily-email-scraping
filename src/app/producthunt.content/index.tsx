...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    while ((pageCount < maxScrollAttempts) && (noChangeCount < maxNoChangeCount)) {
      ...
      // 检查高度和产品数量是否变化
      if (previousHeight === currentHeight && previousProductCount === currentProducts.length) {
        noChangeCount++;
        console.log(`No changes detected ${noChangeCount}/${maxNoChangeCount} times (height: ${currentHeight}, products: ${currentProducts.length})`);
      } else {
      ...
      console.log(
        `Scrolling attempt ${pageCount}/${maxScrollAttempts}, position: ${Math.round(window.scrollY)}/${document.body.scrollHeight}, collecting URLs... length: ${urls.length}`
      );
    }
