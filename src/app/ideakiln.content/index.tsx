...
export default defineContentScript({
  matches: ["https://ideakiln.com/ideas/today"],
  cssInjectionMode: "ui",
  runAt: "document_end",

  async main(ctx) {
    console.log("Content script is running on ideakiln.");
    ...
    const statusPanelId = "ideakiln-status-panel-" + Date.now();
    ...
    // Find div with class containing "IdeasPageShell_center"
    let targetDiv = null;
    const allDivs = document.querySelectorAll("div");
    for (const div of allDivs) {
      const className = div.className;
      if (className && className.includes("IdeasPageShell_center")) {
        targetDiv = div;
        break;
      }
    }
    ...
    if (!targetDiv) {
      console.warn("Could not find IdeasPageShell_center div");
      updateStatus("error", 0, "Could not find target section");
      setTimeout(() => {
        removeStatusPanel();
      }, 3000);
      return;
    ...
    links.forEach((a) => {
      ...
      if (href && href.startsWith("https") && !href.includes("ideas")) {
        urls.push(href);
      }
    });
