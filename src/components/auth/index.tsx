...
const Socials = () => {
  const { mutate } = useMutation({
    ...
    onSuccess: () => {
      // this callback will be called when the betterAuth sign api called successfully, not when the user is authenticated.
      browser.tabs.getCurrent().then((tab) => {
        if (tab?.id) {
          sendMessage(Message.AUTH_SUCCESS, tab.id);
        }
      });
    },
  });
